...
class VIAlu extends Module {
  ...
  // 2026 CIE 大赛自定义：vdot.vv 点积归约（复用 vipu 归约通路）
  val vDotAlu = Module(new VDotFu)
  vDotAlu.io.in := io.in
  ...
  val vdFinal = Mux(opcodeS2.isVmvxs, vs2ExtS2,
    Mux(opcodeS2.isVdot, vDotAlu.io.out.vd,
      Mux(opcodeS2.isReduction, vReduAlu.io.out.vd, vMaskAlu.io.out.vd)))
  io.out.bits.vd := vdFinal
  io.out.bits.vxsat := Mux(opcodeS2.isVdot, vDotAlu.io.out.vxsat, false.B)
